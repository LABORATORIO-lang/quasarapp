  ...
  <q-page class="q-pa-lg text-white bg-grey-10">
    <div class="row items-center justify-between q-mb-xl q-col-gutter-sm">
      <div class="row items-center q-gutter-sm">
        ...
        <div>
          <div class="text-h5 text-weight-bold">Checklists Logística</div>
    ...
    <div class="row q-col-gutter-lg">
      <div v-for="(modelo, index) in modelos" :key="modelo.id" class="col-12 col-sm-4">
        <q-card class="bg-grey-9 text-white custom-card column justify-between">
          ...
          <q-card-actions align="between" class="bg-grey-10 q-pa-sm">
            <div class="row q-gutter-xs">
              <q-btn
                ...
                color="red-5"
                icon="delete"
                @click="solicitarExclusaoModelo(index)"
              >
...
const carregarModelos = async () => {
  $q.loading.show()
  try {
    const querySnapshot = await getDocs(collection(db, 'checklist_logistica'))
    const lista = []
...
function adicionarNovaMaquina() {
  $q.dialog({
    ...
    if (nome.trim()) {
      ...
      router.push(`/admin/editar/checklist_logistica/${slug}`)
    }
  })
}
...
function solicitarExclusaoModelo(index) {
  ...
  $q.dialog({
    ...
    $q.loading.show()
    try {
      await deleteDoc(doc(db, 'checklist_logistica', modeloAtual.id))
